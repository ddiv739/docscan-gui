...
def main():
    ...
    graph.draw_image(data=logo200, location=(0,0))
            ...
            if None not in (start_point, end_point):
                if values['-MOVE-']:
                    for fig in drag_figures:
                        graph.move_figure(drag_figures[0], delta_x, delta_y)
                        graph.update()

                elif values['-RECT-']:
                    prior_rect = graph.draw_rectangle(start_point, end_point,fill_color='green', line_color='red')
                elif values['-CIRCLE-']:
                    prior_rect = graph.draw_circle(start_point, end_point[0]-start_point[0], line_color='green')
                elif values['-LINE-']:
